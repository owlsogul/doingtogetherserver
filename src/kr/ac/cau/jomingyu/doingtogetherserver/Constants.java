package kr.ac.cau.jomingyu.doingtogetherserver;

public class Constants {

	public static final String 
		KEY_DATATYPE = "DATATYPE",
		
		// Register
		KEY_REGISTER = "REGISTER",
		KEY_REGISTER_ID = "ID",
		KEY_REGISTER_PW = "PW",
		KEY_REGISTER_NAME = "NAME",
		KEY_REGISTER_EMAIL = "EMAIL",                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      
		KEY_REGISTER_PHONE = "PHONE",
		KEY_REGISTER_RESULT = "RESULT",
		
		//Login
		KEY_LOGIN = "LOGIN",
		KEY_LOGIN_ID = "ID",
		KEY_LOGIN_PW = "PW",
		KEY_LOGIN_KEY = "KEY",
		KEY_LOGIN_RESULT = "RESULT",
		
		//Upload
		KEY_UPLOAD = "UPLOAD",
		KEY_UPLOAD_ID = "ID",
		KEY_UPLOAD_CONTENT = "CONTENT",
		KEY_UPLOAD_RESULT = "RESULT",
		
		//Download
		KEY_DOWNLOAD = "DOWNLOAD",
		KEY_DOWNLOAD_ID = "ID",
		KEY_DOWNLOAD_CONTENT = "CONTENT",
		KEY_DOWNLOAD_RESULT = "RESULT",
		
		//Share
		KEY_SHARE = "SHARE",
		KEY_SHARE_WRITER_KEY = "WRITER_KEY",
		KEY_SHARE_WRITER_ID = "WRITER_ID",
		KEY_SHARE_WRITE_TIME = "WRITE_TIME",
		KEY_SHARE_TITLE = "TITLE",
		KEY_SHARE_PRIORITY = "PRIORITY",
		KEY_SHARE_PEOPLE = "PEOPLE",
		KEY_SHARE_DUEDATE = "DEU_DATE",
		KEY_SHARE_MEMO = "MEMO",
		KEY_SHARE_IMAGES = "IMAGES",
		//KEY_SHARE_CHEERUP = "CHEERUP"
		KEY_SHARE_RESULT = "RESULT",
		
		// timeline
		KEY_TIMELINE = "TIMELINE",
		KEY_TIMELINE_CONTENTS = "CONTENTS",
		KEY_TIMELINE_WRITER_ID = "WRITER_ID",
		KEY_TIMElINE_WRITE_TIME = "WRITE_TIME",
		KEY_TIMELINE_TITLE = "TITLE",
		KEY_TIMELINE_PRIORITY = "PRIORITY",
		KEY_TIMELINE_PEOPLE = "PEOPLE",
		KEY_TIMELINE_DUE_DATE = "DUE_DATE",
		KEY_TIMELINE_MEMO = "MEMO",
		KEY_TIMELINE_RESULT = "RESULT"
		
		;
	
	public static final String
		TABLE_USER_INFO = "user_info",
		COLUMN_USER_KEY = "user_key",
		COLUMN_USER_ID = "id",
		COLUMN_USER_PW = "pw",
		COLUMN_USER_NAME = "name",
		COLUMN_USER_EMAIL ="email",
		COLUMN_USER_PHONE = "phone"	,
		
		
		TABLE_SHARING_INFO = "sharing_info",
		COLUMN_SHARING_WRITER_KEY = "writer_key",
		COLUMN_SHARING_WRITER_ID = "writer_id",
		COLUMN_SHARING_WRITE_TIME = "write_time",
		COLUMN_SHARING_TITLE = "title",
		COLUMN_SHARING_PRIORITY = "priority",
		COLUMN_SHARING_PEOPLE = "people",
		COLUMN_SHARING_DUEDATE = "due_date",
		COLUMN_SHARING_MEMO = "memo",
		COLUMN_SHARING_IMAGES = "images",
		COLUMN_SHARING_CHEERUP_KEY = "cheerup_key"
		;
	
	
		
	
}
